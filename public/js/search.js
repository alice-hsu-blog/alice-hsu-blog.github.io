(() => {
  ...
  (function() {
    ...
    function createResultElement(result, query, index) {
      ...
      const sourceText = result.summary && keywords.some((k) => result.summary.toLowerCase().includes(k)) ? result.summary : result.content;
    ...
    function findAndHighlight(text, keywords, maxlen) {
      ...
      merged.forEach(([from, to]) => {
        ...
        result += '<mark class="bg-primary/20 text-primary px-1 rounded font-medium">' + text.slice(safeFrom, safeTo) + "</mark>";
        cursor = safeTo;
      });
      if (cursor < end) {
